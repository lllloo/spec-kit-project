...
**Language/Version**: PHP 8.4 (Laravel 12，實裝 deps 經 composer platform_check 要求 ≥ 8.4) / TypeScript 6.0 (React 19，Vite template 預設)
...
├── .docker/Dockerfile                      # php:8.4-cli + pdo_mysql/gd/zip/intl + composer（dev image）
├── app/
...
docker-compose.yml                          # mysql:8.4 + mailpit + redis + app（Laravel dev server）+ queue（共用 image）
```
